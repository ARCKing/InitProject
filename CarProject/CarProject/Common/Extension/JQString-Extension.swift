...
extension String{
    ...
    // 从第几个位置开始截取后面的字符串
    func substringStartIndex(index: Int) -> String? {
        guard index > 0 else {
            return nil
        }
        if index >= self.count {
        ...
        let startSlicingIndex = self.index(self.startIndex, offsetBy: index)
        return String(self[startSlicingIndex...])
    }
    
    // 电话号码打*号
    func phoneNumberAddStar() -> String {
        let tempStr = self
        if tempStr.count < 11 { // 如果不是电话号码则直接返回
            return tempStr
        }
        // 截取前面的三个字符
        let firstStr = tempStr.substringToIndex(3)
        let secondStr = tempStr.substringStartIndex(index: 7)

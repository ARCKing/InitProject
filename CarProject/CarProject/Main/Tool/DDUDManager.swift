...
class DDUDManager {
    ...
    // 保存融云的token
    func saveRongCloudToken(deviceToken: String) -> Bool {
        UserDefaults.ServerInfo.set(value: deviceToken, forKey: .rongCloudToken)
        return UserDefaults.standard.synchronize()
    }

    func rongCloudToken() -> String {
        
        if let token = UserDefaults.ServerInfo.string(forKey: .rongCloudToken) {
            return token
        }else{
            return ""
        }
    ...
    // 获取登录融云的用户信息
    func getRongCloudUserInfo() -> [String: Any] {
        if let dict = UserDefaults.ServerInfo.dict(forKey: .rongCloudUserInfo) {
            return dict
        }else{
            return [:]
        }
    ...
    // 保存登录融云的用户信息
    func saveRongCloudUserInfo(uid: String, name: String?, headImgStr: String?) {
        var dict = [String: String]()
        dict["uid"] = uid
        dict["name"] = name ?? ""
        dict["img"] = headImgStr ?? ""
        UserDefaults.ServerInfo.set(value: dict, forKey: .rongCloudUserInfo)
        UserDefaults.standard.synchronize()
    }

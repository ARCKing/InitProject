...
//    static let navigationRightItemAction = #selector(DDBaseViewController.navigationRightItemAction(btn:))
//
//}
...
class DDBaseViewController: UIViewController {
    ...
    var networkReachabilit = true {
        didSet(newValue){
            networkReachabilit = newValue;
        }
    }
    ...
    func baseViewWillAppearSetNavBar() {
        ...
        navigationController?.navigationBar.barTintColor = DDGlobalNavBarColor()
    ...
    func addNavBarBackButton(imageName: String?, callBack: @escaping NavBarItemBlock) {
        ...
        let rightButton = UIButton.init(type: .custom)
        rightButton.imageEdgeInsets = UIEdgeInsets.init(top: 0, left: -15, bottom: 0, right: 0)
        rightButton.frame = CGRect.init(x: 0, y: 0, width: 40, height: 40)
        rightButton.setImage(UIImage.init(named: imageName ?? " "), for: .normal)
    ...
    ///NavBar分割线设置
    func addNavBarSeparation(with color:UIColor){
        let line = UIView.init(frame: CGRect.init(x: 0, y: 43, width: Screen.width, height: 1));
        line.backgroundColor = color;
        self.navigationController?.navigationBar.addSubview(line);
        self.navigationController?.navigationBar.bringSubview(toFront: line);
    }
    
    ///获取当前网络状态-NetworkReachabilitStatue
    func NetworkReachabilitStatue() -> Bool{
        let statue = networkManager?.isReachable;
        if let _ = statue {
            networkReachabilit = statue!
            return statue!;
        }else{
            return false;
        }
